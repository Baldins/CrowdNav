...
def gp_computation_mc(frame, num_peds, x, y, x_obs, y_obs, \
        ...
        num_intents, num_var_samples, var_ratio, dwa, cov_eps):
    ...
    if ped == num_peds:#Robot GP
      ...
      mu_linear_conditioned_x[ped], cov_linear_conditioned_x[ped] = \
          gp_x[ped].predict(x_obs[ped], \
                grid[(len(x_obs[ped])-1):Tdex[ped]+(len(x_obs[ped])-1)], \
                return_cov=True)
      ...
      x_obs_times[ped] = np.linspace(0, obs_duration_ped, obs_duration_ped+1)
      y_obs_times[ped] = np.linspace(0, obs_duration_ped, obs_duration_ped+1)

      x_obs_times_un[ped] = np.linspace(0, obs_duration_ped-1, obs_duration_ped)
      y_obs_times_un[ped] = np.linspace(0, obs_duration_ped-1, obs_duration_ped)

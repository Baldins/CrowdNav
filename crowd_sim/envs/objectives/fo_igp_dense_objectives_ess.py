...
def ll(x, T, robot_mu_x, robot_mu_y, ped_mu_x, ped_mu_y, cov_robot_x, cov_robot_y, \
  ...
  quad_ped_mu_y = np.dot((x[(n+1)*T:(n+2)*T]-ped_mu_y).T, np.dot(\
                                    inv_cov_ped_y, x[(n+1)*T:(n+2)*T]-ped_mu_y))
  ...
  quad_robot_ped_x = np.multiply(vel_x_2, one_over_cov_sum_x)
  quad_robot_ped_y = np.multiply(vel_y_2, one_over_cov_sum_y)
...
def d_ll(x, T, robot_mu_x, robot_mu_y, ped_mu_x, ped_mu_y, \
  ...
  quad_robot_x = np.multiply(one_over_cov_sum_x, vel_robot_x_2)
  quad_robot_y = np.multiply(one_over_cov_sum_y, vel_robot_y_2)
  ...
  alpha_x = np.multiply(X, np.multiply(vel_robot_x, one_over_cov_sum_x))
  alpha_y = np.multiply(X, np.multiply(vel_robot_y, one_over_cov_sum_y))
...
#        X AND Y COMPONENT OF PED DERIVATIVE
  ...
  quad_ped_x = np.multiply(one_over_cov_sum_x, vel_ped_x_2)
  quad_ped_y = np.multiply(one_over_cov_sum_y, vel_ped_y_2)
  ...
  alpha_x = np.multiply(X, np.multiply(vel_ped_x, one_over_cov_sum_x))
  alpha_y = np.multiply(X, np.multiply(vel_ped_y, one_over_cov_sum_y))

  d_alpha[n*T:(n+1)*T] = -np.sum(alpha_x, axis=1)
  d_alpha[(n+1)*T:(n+2)*T] = -np.sum(alpha_y, axis=1)

  d_beta[n*T:(n+1)*T] = -np.dot(x[n*T:(n+1)*T]-ped_mu_x, \
                                                           inv_cov_ped_x)
  d_beta[(n+1)*T:(n+2)*T] = -np.dot(x[(n+1)*T:(n+2)*T]-ped_mu_y, \
                                                           inv_cov_ped_y)
...
def dd_ll(x, T, robot_mu_x, robot_mu_y, ped_mu_x, ped_mu_y, \
          cov_robot_x, cov_robot_y, inv_cov_robot_x, inv_cov_robot_y, \
          cov_ped_x, cov_ped_y, inv_cov_ped_x, inv_cov_ped_y, \
          one_over_cov_sum_x, one_over_cov_sum_y, normalize):
  T = np.size(robot_mu_x)
  ...
  sum_d_alpha = [0. for _ in range(4*T)]
  ...
  one_over_cov_x_y = np.multiply(one_over_cov_sum_x, one_over_cov_sum_y)
  ...
  d_alpha_x = np.multiply(X, one_over_cov_sum_x)
  d_alpha_x = np.add(d_alpha_x, -np.multiply(X_plus_X2, np.power(\
                              np.multiply(vel_robot_x, one_over_cov_sum_x), 2)))
  d_alpha_y = np.multiply(X, one_over_cov_sum_y)
  d_alpha_y = np.add(d_alpha_y, -np.multiply(X_plus_X2, np.power(\
                              np.multiply(vel_robot_y, one_over_cov_sum_y), 2)))
...
#      PED DIAGONALS
  ...
  d_alpha_x = np.multiply(X, one_over_cov_sum_x)
  d_alpha_x = np.add(d_alpha_x, -np.multiply(X_plus_X2, np.power(\
                                np.multiply(vel_ped_x, one_over_cov_sum_x), 2)))
  d_alpha_y = np.multiply(X, one_over_cov_sum_y)
  d_alpha_y = np.add(d_alpha_y, -np.multiply(X_plus_X2, np.power(\
                                np.multiply(vel_ped_y, one_over_cov_sum_y), 2)))

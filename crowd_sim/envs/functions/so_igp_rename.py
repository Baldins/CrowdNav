    ...
    if conditioned:
      ...
      if linear:
        one_over_ped_cov_x[ped] = \
                  np.diag(np.power(np.diag(cov_linear_conditioned_x[ped]), -1))
        one_over_ped_cov_y[ped] = \
                  np.diag(np.power(np.diag(cov_linear_conditioned_y[ped]), -1))
      else:
        one_over_ped_cov_x[ped] = np.power(cov_linear_conditioned_x[ped], -1)
        one_over_ped_cov_y[ped] = np.power(cov_linear_conditioned_y[ped], -1)
    else:
